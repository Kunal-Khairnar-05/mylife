...
const MagicButton = ({title, icon, position, handleClick, otherClasses}
  ...
  <span className={`inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-slate-950 px-3 py-1 font-medium text-white backdrop-blur-3xl gap-2 ${otherClasses}}`}>
    {position === 'left' && icon}
    {title}
    {position === 'right' && icon}
  </span>
